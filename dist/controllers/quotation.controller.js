...
exports.QuotationController = {
    ...
    quickCreate: async (req, res) => {
        try {
            ...
            const prod = await product_model_1.ProductModel.findById(productId).select('_id');
            if (!prod)
    ...
    createOrGetCart: async (req, res) => {
        try {
            ...
            if (!userId)
                return res.status(401).json({ message: 'Unauthorized' });
            let cart = await quotation_model_1.QuotationModel.findOne({ user: userId, status: 'carrito' }).populate('items.product');
            if (!cart) {
    ...
    addItem: async (req, res) => {
        try {
            ...
            if (!prod)
                return res.status(404).json({ message: 'Product not found' });
            const quotation = await quotation_model_1.QuotationModel.findById(quotationId);
            if (!quotation)
    ...
    adminSetQuote: async (req, res) => {
        try {
            ...
            const quotation = await quotation_model_1.QuotationModel.findById(id).populate('user', 'email');
            if (!quotation)
                return res.status(404).json({ message: 'Quotation not found' });
            ...
            const ownerEmail = quotation.user?.email;
            if (ownerEmail) {
    ...
    listAll: async (_req, res) => {
        try {
            const quotations = await quotation_model_1.QuotationModel.find()
                .sort({ createdAt: -1 })
